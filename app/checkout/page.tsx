...
export default function CheckoutPage() {
  ...
  const initialNominalId = searchParams.get("nominal");
  ...
  useEffect(() => {
    if (slug) {
      // Ambil detail game (termasuk daftar harga terbaru dari DB)
      ...
      // Kalau gak ada slug, tendang ke home
      router.push("/");
    }
  }, [slug, router]);
  ...
  const paymentMethods = [
    ...
    },
    {
      category: "QRIS",
      icon: <QrCode size={20} />,
      methods: [{ id: "qris", name: "QRIS", fee: 0 }],
    },
    {
      category: "Minimarket",
      icon: <Store size={20} />,
  ...
  // --- LOGIC PERHITUNGAN (Moved Down to access state) ---

  // 1. Cari nominal yang dipilih user dari list game yang sudah di-fetch
  const selectedItem = game?.nominals.find(
    (n: any) => n.id === initialNominalId
  );
  ...
  if (!selectedItem)
    return (
      <div className="min-h-screen bg-background flex items-center justify-center text-white">
        Item invalid atau harga berubah.
      </div>
    );

  // 3. Cari Payment Method yang dipilih
  ...
  const total = itemPrice + adminFee;
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Redirect ke halaman cek transaksi (nanti kita bikin halaman ini otomatis baca invoice)
        router.push('/cek-transaksi'); 
      } else {
      ...
      <section className="pt-28 pb-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="glass p-6 rounded-3xl border border-white/10 sticky top-28">
                <h2 className="text-lg font-bold text-white mb-6">
                  {dict.checkout.order_detail_title}
                </h2>
                ...
                <div className="border-t border-white/10 pt-4 space-y-3 mb-6">
                  <div className="flex justify-between text-sm">
                    <span className="text-white/50">
                      {dict.checkout.item_price}
                    </span>
                    <span className="text-white">{formatPrice(itemPrice)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-white/50">
                      {dict.checkout.admin_fee}
                    </span>
            ...
            <button
  onClick={handlePayment} // <-- Tambah ini
  disabled={!selectedPayment || loading} // <-- Biar gak bisa diklik kalau loading/belum pilih
